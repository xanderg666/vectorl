...
# SQL definitions for Index Management
SQL_DROP_INDEX = f"DROP INDEX idx_vector_{TABLE_NAME}"
...
def manage_index(db, action):
    """Gestiona el ciclo de vida del índice vectorial"""
    if action == "drop":
        logger.info(f"Intentando eliminar el índice 'idx_vector_{TABLE_NAME}' antes de la ingesta...")
        try:
            db.execute_dml(SQL_DROP_INDEX)
            logger.info("✓ Índice eliminado correctamente.")
        except oracledb.DatabaseError as e:
            if 'ORA-01418' in str(e): # Index does not exist
                logger.info("El índice no existía, continuando...")
            else:
                logger.warning(f"Advertencia al eliminar índice: {e}")
    
    elif action == "create":
        logger.info(f"Recreando el índice 'idx_vector_{TABLE_NAME}' después de la ingesta...")
        try:
            db.execute_dml(SQL_CREATE_INDEX)
            logger.info("✓ Índice creado correctamente.")
        except Exception as e:
            logger.error(f"Error crítico al crear el índice: {e}")
            raise


def process_and_ingest_files():
    """Procesa archivos markdown y los ingesta en formato GenAI"""
    logger.info("Iniciando proceso de ingesta en formato GenAI (FIXED)...")
    ...
    # --- DROP INDEX BEFORE INGESTION ---
    manage_index(db, "drop")
    # -----------------------------------
    ...
    # --- RECREATE INDEX AFTER INGESTION ---
    manage_index(db, "create")
    # --------------------------------------
